...
const createBetInContract = async (bet) => {
    try {
        ...
        return {
            success: true,
            betId: betTx.betId,
            explorerLink: `${networkId === 'testnet' ? 'https://sepolia.basescan.org/tx/' : 'https://basescan.org/tx/'}${betTx.hash}`
        };
    } catch (e) {
...
const resolveBetInContract = async (bet, winner) => {
    try {
        ...
        const resolutionTx = await bankrContract.resolveBet(
            bet.betId,
            winner
        );
        ...
        return {
            success: true,
            explorerLink: `${networkId === 'testnet' ? 'https://sepolia.basescan.org/tx/' : 'https://basescan.org/tx/'}${resolutionTx.hash}`
        };
    } catch (e) {
...
const processSettlements = async () => {
    ...
    if (bet.settlementTweet && bet.settlementTweet.text) {
        ...
        } else if (tweetText.includes(`@${bet.opponentUsername.toLowerCase()} won`)) {
    ...
    if (winnerAddress) {
        ...
        if (resolution.success) {
            ...
            const conversationId = await getConversationId(client, bet.settlementTweet.id);
            let replyId = bet.settlementTweet.id;
            
            if (conversationId) {
                const latestTweet = await getLatestConversationTweet(client, conversationId);
                if (latestTweet) {
        ...
        console.log('Could not determine winner, waiting for clearer settlement instruction');
        bet.settlementAttempt = (bet.settlementAttempt || 0) + 1;
...
const processAddressRequest = async () => {
    ...
    try {
        const conversationId = await getConversationId(client, bet.addressRequestTweetId);
            ...
            for await (const reply of replies) {
                if (reply.author_id === process.env.BANKRBOT_ID) {
                    ...
                    if (addresses && addresses.length >= 2) {
                        ...
                        console.log('Found addresses:', bet.creatorAddress, bet.opponentAddress);
...
export default async function search(req, res) {
        ...
        if (pass === process.env.RESTART_PASS) {
            ...
            if (manualSettle) {
                ...
                if (betIndex >= 0) {
                    ...
                    const winnerAddress = winner === 'creator' 
                        ? bet.creatorAddress 
                        : bet.opponentAddress;
                    
                    resolveBetInContract(bet, winnerAddress)
                        .then(res => console.log('Manual settlement result:', res));
                }
            }
        }
    } catch (e) {
    ...
    const start_time = lastTweetTimestamp > 0
    ...
    const settleTweetGenerator = await client.v2.search('@funnyorfud "settle bet"', {
        start_time,
        'tweet.fields': 'author_id,created_at,referenced_tweets,author_username',
    });
    ...
    for await (const tweet of settleTweetGenerator) {
        ...
        const activeBets = pendingSettlement.filter(bet => 

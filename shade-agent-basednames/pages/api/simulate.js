...
const generateDepositAddress = (username, tweetId) => {
  ...
  const hash = [...(username + tweetId).slice(0, 38)].reduce(
...
const fakeTweets = [
  ...
  // Bank bot responses with addresses (will be automatically generated in simulator)
  {
    ...
    text: '@alex @bob Addresses: ' + SIMULATED_ADDRESSES.alex + ' and ' + SIMULATED_ADDRESSES.bob,
  ...
  },
  {
    ...
    author_id: 'bankrbot_id',
    author_username: 'bankrbot',
    text: '@charlie @diana Addresses: ' + SIMULATED_ADDRESSES.charlie + ' and ' + SIMULATED_ADDRESSES.diana,
...
const getLatestConversationTweet = async (client, conversationId) => {
  console.log(`[Simulation] Getting latest tweet in conversation ${conversationId}`);
  const tweetId = conversationId.replace('conversation-', '');
  // Find all tweets that reference this tweet
  const relatedTweets = fakeTweets.filter(tweet => 
...
const searchTweets = async (query, options = {}) => {
  ...
  if (query.includes('Wagered')) {
    matchingTweets = fakeTweets.filter(tweet => 
...
const replyToTweet = async (text, replyToId) => {
  console.log(`[Simulation] Replying to tweet ${replyToId} with text: ${text.slice(0, 50)}...`);
...
const simulateBlockchain = {
  ...
  getBalance: async ({ address }) => {
    ...
    const creatorBet = pendingDeposits.find(bet => bet.creatorDepositAddress === address);
    const opponentBet = pendingDeposits.find(bet => bet.opponentDepositAddress === address);
  ...
  getBankrContract: async () => {
    ...
    return {
      createBet: async (description, creator, opponent, resolver) => {
        console.log(`[Simulation] Creating bet between ${creator} and ${opponent}`);
      ...
      resolveBet: async (betId, winner) => {
        console.log(`[Simulation] Resolving bet ${betId} with winner ${winner}`);
...
const processReplies = async () => {
  ...
  try {
    ...
    if (response?.data) {
      ...
      console.log(`[Simulation] Added bet to pendingAddressRequest queue: ${JSON.stringify(bet, (key, value) => typeof value === 'bigint' ? value.toString() : value)}`);
    }
  } catch (e) {
...
const processAddressRequest = async () => {
  ...
  bet.creatorAddress = SIMULATED_ADDRESSES[bet.creatorUsername] || SIMULATED_ADDRESSES.alex;
  bet.opponentAddress = SIMULATED_ADDRESSES[bet.opponentUsername] || SIMULATED_ADDRESSES.bob;
  
  console.log(`[Simulation] Found addresses: ${bet.creatorAddress}, ${bet.opponentAddress}`);
...
const processAddressConfirmation = async () => {
  ...
  console.log(`[Simulation] Processing address confirmation for bet: ${bet.id}`);
  ...
  if (response?.data) {
    ...
    console.log(`[Simulation] Added bet to pendingDeposits queue: ${JSON.stringify({ ...bet, stake: bet.stake.toString() })}`);
  }
};
...
const processDeposits = async () => {
  ...
  console.log(`[Simulation] Creator deposit attempts: ${bet.creatorDepositAttempt}`);
  console.log(`[Simulation] Opponent deposit attempts: ${bet.opponentDepositAttempt}`);
  ...
  if (!creatorDeposited && bet.creatorDepositAttempt >= 2) { // Simulate deposit after 2 attempts
    try {
      const creatorBalance = await simulateBlockchain.getBalance({ 
        address: bet.creatorDepositAddress 
      });
      console.log(`[Simulation] Creator balance: ${simulateBlockchain.formatBalance(creatorBalance)}`);
  ...
  if (!opponentDeposited && bet.opponentDepositAttempt >= 3) { // Simulate deposit after 3 attempts (slightly later)
    try {
      const opponentBalance = await simulateBlockchain.getBalance({ 
        address: bet.opponentDepositAddress 
      });
      console.log(`[Simulation] Opponent balance: ${simulateBlockchain.formatBalance(opponentBalance)}`);
  ...
  if (creatorDeposited && opponentDeposited) {
    console.log('[Simulation] Both parties deposited, creating bet in contract');
    
    const betResult = await simulateBlockchain.getBankrContract()
      .then(contract => contract.createBet(
        bet.description,
        bet.creatorAddress,
        bet.opponentAddress,
...
const processSettlements = async () => {
  ...
  const settlementTweets = fakeTweets.filter(tweet => 
    tweet.text.toLowerCase().includes('settle bet') && 
    (tweet.author_username === bet.creatorUsername || tweet.author_username === bet.opponentUsername)
  );
  ...
  if (settlementTweets.length > 0) {
    ...
    if (resolution) {
      ...
      await replyToTweet(
        `Bet settled! 🎉\n\n@${winnerUsername} won ${simulateBlockchain.formatBalance(bet.stake * 2n * 99n / 100n)} ETH\n\nReason: ${settlementReason}\n\nTx: https://basescan.org/tx/${resolution.hash}`,
      ...
      console.log(`[Simulation] Bet settled and moved to completedBets: ${bet.id}`);
    }
  } else {
    // No settlement tweet yet, put back in queue
    console.log('[Simulation] No settlement tweet found, putting bet back in queue');
    pendingSettlement.push(bet);
  }
};
...
const runSimulation = async () => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log(`[Simulation] Deposit cycle ${i+1} completed. Bets remaining: ${pendingDeposits.length}`);
    await sleep(SIMULATION_STEP_DELAY);
  }
